angular.module('firstlife.controllers')
    .controller('EditorCtrl', ['$state', '$scope', '$location', '$ionicPopup', '$window', '$filter', '$ionicLoading', '$log', '$ionicSlideBoxDelegate', '$timeout', '$ionicScrollDelegate', 'myConfig', 'EntityService', 'ThingsService', 'AuthService', 'PlatformService', 'SearchService', function ($state, $scope, $location, $ionicPopup, $window, $filter, $ionicLoading, $log, $ionicSlideBoxDelegate, $timeout, $ionicScrollDelegate, myConfig, EntityService, ThingsService, AuthService, PlatformService, SearchService) {
        ...
        // gestione validita' ng-form delle categorie per il passo next
        $scope.catStepInvalid = function () {
            var cats = $filter('filter')(_this.categories, $scope.isCatRelevant);
            $log.log(_this.form.wizardform);
            $timeout(function (){
                $scope.isCatStepInvalid = cats.reduce(function (check, space) {
                $log.log(check, space);

                $log.log(_this.form.wizardform["cats" + space.category_space].$invalid);
                return _this.form.wizardform["cats" + space.category_space].$invalid || check;

            }, false);
            $log.log('invalid?', $scope.isCatStepInvalid);
        }, 100);
    };
...
function initEntity(entity_type) {
    ...
    var typeIndex = _this.types.list.map(function (e) {
        return e.key;
    }).indexOf(entity_type);
    ...
    if (parentId && parentType) {
        ...
        var index = types.map(function (e) {
            return e.key
        }).indexOf(parentType)
...
function initDoorTime() {
    ...
    angular.extend($scope.checkList.door_time, {
        template: {
...
function doorTimeCallback(val) {
    if (val) {
        ...
        _this.wizard.dataForm.door_time = null;
    }
}
function initDuration() {
    ...
    angular.extend($scope.checkList.close_time, {
        template: {
            inputEpochTime: inputEpochTime,// ((new Date()).getHours() * 60 * 60),  //Optional
            step: 15,  //Optional
            format: 24,  //Optional
