angular.module('firstlife.factories')
    .factory('entityFactory', ['$http', '$q',  '$rootScope', 'myConfig', 'categoriesFactory','MemoryFactory', function($http, $q,  $rootScope, myConfig, categoriesFactory,MemoryFactory) {
        ...
        return {
            ...
            getBBoxPlace: function(bbox,reset) {
                var deferred = $q.defer();
                var urlId = urlPlace.concat(format);
                var eType = 'FL_PLACES';
                var checkRst=checkBboxHistory(bbox, eType);
                ...
                console.log("EntityFactory, getBBoxPlace, check history: ",self.bboxHistory[eType],self.bboxHistory[eType][checkRst], self.BBOX_TIMEOUT);
                ...
                if (self.config.behaviour.marker_cache && checkRst > -1 && !reset) {
                    ...
                    var bboxTmp=self.bboxHistory[eType][checkRst];
                    ...
                    if (bboxTmp.timestamp.getTime()+self.BBOX_TIMEOUT<(new Date()).getTime()) {
                        ...
                        var req = {
                            url: urlId.concat(bboxParamsString),
                            method: 'GET',
                            headers:{"Content-Type":"application/json"},
                            data:bbox
                        };
                        $http(req)
                            ...
                            //console.log("PlaceFactory, getBBoxPlace + since, get, response: ",response);
                            console.log("PlaceFactory, getBBoxPlace con since, rensponse: ", response);
                            entitiesToMarker(response.data.data, eType).then(
                                function(markers){
                                    //salvo i marker senza dettagli
                                    updateMarkersList(markers,false);
                                    deferred.resolve(markers);
                                },
                                function(err){deferred.reject(err);});

                        },function(response) {
                            console.log("getBBoxPlace, errore: ", response);
                            deferred.reject(response);
                        });
                    ...
                    $http(req)
                        .then(function(response) {
                        //console.log("PlaceFactory, getBBoxPlace no since, get, response: ", response);
                        //angular.merge(self.geoJSON,response.data.data[0]);
                        console.log("PlaceFactory, getBBoxPlace, rensponse: ", response);
                        entitiesToMarker(response.data.data,eType).then(
                            function(markers){
                                //salvo i marker senza dettagli
                                updateMarkersList(markers,false);
            ...
            getBBoxEvent: function(bbox,reset) {
                var deferred = $q.defer();
                var urlId = urlEvent.concat(format);
                var eType = 'FL_EVENTS';
                var checkRst=checkBboxHistory(bbox, eType);
                var bboxParamsString = bboxBaseParams;
                //query senza since
                ...
                if (self.config.behaviour.marker_cache && checkRst > -1 && !reset) {
                    ...
                    if (bboxTmp.timestamp.getTime()+self.BBOX_TIMEOUT<(new Date()).getTime()) {
                        ...
                        $http(req)
                            .then(function(response) {
                            //console.log("PlaceFactory, getBBoxPlace + since, get, response: ",response);
                            //console.log("PlaceFactory, getBBoxPlace con since, rensponse, merge with geoJSON: ", self.geoJSON);
                            entitiesToMarker(response.data.data, eType).then(
                                function(markers){
                                    //salvo i marker senza dettagli
                                    updateMarkersList(markers,false);
                    ...
                    $http(req)
                        ...
                        //console.log("PlaceFactory, getBBoxPlace no since, get, response: ", response);
                        //angular.merge(self.geoJSON,response.data.data[0]);
                        //console.log("PlaceFactory, getBBoxPlace, rensponse, merge with geoJSON: ", self.geoJSON);
                        entitiesToMarker(response.data.data,eType).then(
                            function(markers){
                                //salvo i marker senza dettagli
                                updateMarkersList(markers,false);
                        ...
                        console.log("getBBoxEvent, errore: ", response);
                        deferred.reject(response);
                    });
                }
                return deferred.promise;
        ...
        function updateMarkersList(newMarkers,details){
            for(el in newMarkers){
                //console.log("updateMarkersList, place caricato ",el, " anche nella lista dettagli? ",details); 
                self.markerList[el] = newMarkers[el]; 

                if(details){
                    self.markerDetailsList[el] = newMarkers[el]; 
                }
            }
        }
        ...
        //private function
        function entitiesToMarker(data,eType) {
            var deferred = $q.defer();
            if(data){
                ...
                //console.log("entitiesToMarker placeFact", data, featureCollection, placeList);
                if(self.mainCategories){
                    var markers = [];
                    for (i = 0; i < entityList.length; i++) {
                        //console.log("place da trasformare in marker", placeList[i], " numero: ",i);
                        if(entityList[i] && entityList[i] != null && entityList[i] != 'undefined'){
                            var marker = markerCreate(entityList[i],eType);
                            if(marker)
                                markers[entityList[i].id] = marker;
                        }
                    }
                    deferred.resolve(markers);
                }else{
        ...
        //private function
        function entityToMarker(data,eType) {
            var deferred = $q.defer();
            ...
            if(data === "22P02"){
                ...
                if(features && Array.isArray(features) && features.length > 0){
                    entity = features[0];
                    if(self.mainCategories){
                        var marker = markerCreate(entity,eType);
                        if(marker)
                            deferred.resolve(marker);
                        else
        ...
        function markerCreate(entity, eType) {
            //bugfix 
            if(Array.isArray(entity.properties))
                entity.properties = entity.properties[0];
            
            var colors = myConfig.colors,
                categories = self.mainCategories,
                catIndex = parseInt(categories.map(function(e) { return e.id; }).indexOf(entity.properties.categories[0].categories[0])),
                category = categories[catIndex],
                entity_type = eType ? eType : entity.properties.entity_type,
                type_info = self.config.types.list[self.config.types.list.map(function(e){return e.key;}).indexOf(entity_type)],
                category_list = [];
            ...
            //console.log("markerCreate, entity: ", entity, entity.properties.categories.length, category, catIndex);
            var marker = {
                ...
                icon: {
                    ...
                    iconSize:  [30, 30],
                    iconAnchor:   [10, 30],
                    html : '<div class="pin-marker" style="background-color:'+ category.color+'"></div>'+
                    '<div class="icon-box"><i class="icon ' + category.icon + '"></i></div>'
                },
        ...
        function checkBboxHistory(bbox,type) {
            //console.log("EntityFactory, checkBboxHistory, bbox e type: ",bbox,type, " esiste la history? ",self.bboxHistory);
            
            if (!bbox.parent_bbox) {
                var toDelete = [];
                for (i = 0; i < self.bboxHistory[type].length; i++) {
                    var bbtmp = self.bboxHistory[type][i];
                    if (bbtmp.valid) {
                        if (bbox.ne_lat <= bbtmp.ne_lat && bbox.ne_lng <= bbtmp.ne_lng && bbox.sw_lat >= bbtmp.sw_lat && bbox.sw_lng >= bbtmp.sw_lng)
                            ...
                            self.bboxHistory[type][i].valid = false;
                    }
                ...
                bbox.valid = true;
                self.bboxHistory[type].push(bbox);
                ...
                if ((self.bboxHistory[type].length == 0)||(self.bboxHistory[type].length > 0 && self.bboxHistory[type][0].hash != bbox.parent_bbox.hash)) {
                    bbox.parent_bbox.timestamp = new Date();
                    bbox.parent_bbox.valid = true;
                    self.bboxHistory[type].unshift(bbox.parent_bbox);
                }
                //console.log("NEW history: " + self.bboxHistory[type].length);
                //console.log(self.bboxHistory[type]);
